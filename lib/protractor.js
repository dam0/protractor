...
/**
 * Wrapper around webdriver.WebDriver.Navigation.prototype.refresh so that mock 
 * modules are loaded before Angular.
...
Protractor.prototype.refresh = function(opt_timeout) {
  ...
  // DEFER_LABEL signals to angular to stop bootstrapping 
  // until angular.resumeBootstrap is called.
  self.driver.executeScript(
      'window.name = "' + DEFER_LABEL + '" + window.name; return window.name;')
      .then(function(oldWindowname) {

    self.driver.navigate().refresh();

    // When Angular finishes initialization, it removes the DEFER_LABEL from 
    // window.name; wait until this happens. 
    self.driver.wait(function() {
      return self.driver.executeScript('return window.name;').then(function(windowName) {
        return windowName == oldWindowname.replace(DEFER_LABEL, "");
      })
    }, timeout * 1000, 'Timed out waiting for angular to load (possibly ' 
      + 'because page did not load or page is not angular)');
  });

  // At this point, Angular will pause for us, until angular.resumeBootstrap
  // is called.
  for (var i = 0; i < self.moduleScripts_.length; ++i) {
    name = self.moduleNames_[i];
    self.driver.executeScript(self.moduleScripts_[i]).
      then(null, function(err) {
...
/**
 * Wrapper around webdriver.WebDriver.Navigation.prototype.to so that mock 
 * modules are loaded before Angular (See Protractor.prototype.refresh)
 *
 * General notes for navigation commands: get, back, and forward
 * ## We need to consider cases for navigating to the following:
 * - different url path (i.e. different website or different resource)
 * --> Different view; Browser reloads Angular (all mock angular modules resets)
 * - same url path but different hash (i.e. different page of same Angular app)
 * --> Different view; Browser does not reload Angular (angular already bootstrapped)
 * - same href (i.e. same page of Angular app)
 * --> Same view; Browser does not reload Angular (angular already bootstrapped)
 * To generalize into single case, every navigation command loads a page twice: 
 * First with the raw navigation command (at which time, no mock modules 
 * will load). Then it calls refresh to reload angular and mock modules.
 *
...
Protractor.prototype.get = function(destination, opt_timeout) {
  ...
  // Should we check here to see if changing the URL would normally not
  // cause a full reload?
...
/**
 * Wrapper around webdriver.WebDriver.Navigation.prototype.back so that mock 
 * modules are loaded before Angular (See Protractor.prototype.refresh)
 *
 * @param {number=} opt_timeout Number of seconds to wait for Angular to start.
 */
Protractor.prototype.back = function(opt_timeout) {
  var timeout = opt_timeout || 10;
  var self = this;
  this.driver.executeScript(
      'window.name = "' + DEFER_LABEL + '" + window.name;');
...
/**
 * Wrapper around webdriver.WebDriver.Navigation.prototype.forward so that mock 
 * modules are loaded before Angular (See Protractor.prototype.refresh)
 *
 * @param {number=} opt_timeout Number of seconds to wait for Angular to start.
 */
Protractor.prototype.forward = function(opt_timeout) {
  this.driver.navigate().forward();
  return this.refresh(opt_timeout);
}; 
